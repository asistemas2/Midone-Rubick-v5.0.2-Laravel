...
@section('subhead')
    <title>Editar Estado de Equipo - Parametrización ZFP</title>
@endsection

@section('subcontent')
    <div class="intro-y mt-8 flex items-center">
        <h2 class="mr-auto text-lg font-medium">Editar Estado: {{ $estado->nombre }}</h2>
    </div>
    ...
    <div class="mt-5 grid grid-cols-12 gap-6">
        <div class="intro-y col-span-12 lg:col-span-6">
            <div class="intro-y box p-5">
                <form action="{{ route('parametrizacion.estados_equipo.update', $estado) }}" method="POST">
                    @csrf @method('PUT')
                    <div>
                        <x-base.form-label for="nombre">Nombre <span class="text-danger">*</span></x-base.form-label>
                        <x-base.form-input class="w-full @error('nombre') border-danger @enderror" id="nombre" name="nombre" type="text" value="{{ old('nombre', $estado->nombre) }}" required />
                    ...
                    <div class="mt-3">
                        ...
                        <x-base.form-input class="w-full @error('codigo') border-danger @enderror" id="codigo" name="codigo" type="text" value="{{ old('codigo', $estado->codigo) }}" required />
                    ...
                    <div class="mt-3">
                        ...
                        <div class="flex items-center gap-3">
                            <input type="color" id="color_hex" name="color_hex" value="{{ old('color_hex', $estado->color_hex) }}" class="h-10 w-14 cursor-pointer rounded border border-slate-200 p-1" />
                            <span id="color_hex_label" class="text-sm text-slate-500">{{ old('color_hex', $estado->color_hex) }}</span>
                        </div>
                    ...
                    <div class="mt-3">
                        ...
                        <x-base.form-switch class="mt-2">
                            <x-base.form-switch.input type="checkbox" name="activo" value="1" {{ old('activo', $estado->activo) ? 'checked' : '' }} />
