...
@section('subhead')
    <title>Tipos de Inmueble - Parametrización ZFP</title>
@endsection

@section('subcontent')
    <h2 class="intro-y mt-10 text-lg font-medium">Gestión de Tipos de Inmueble</h2>
    <div class="mt-5 grid grid-cols-12 gap-6">
        <div class="intro-y col-span-12 mt-2 flex flex-wrap items-center sm:flex-nowrap">
            <a href="{{ route('parametrizacion.tipos_inmueble.create') }}">
            ...
            <div class="mx-auto hidden text-slate-500 md:block">
                Mostrando {{ $tiposInmueble->firstItem() ?? 0 }} a {{ $tiposInmueble->lastItem() ?? 0 }} de {{ $tiposInmueble->total() }} registros
            </div>
            <div class="mt-3 w-full sm:mt-0 sm:ml-auto sm:w-auto md:ml-0">
                <form action="{{ route('parametrizacion.tipos_inmueble.index') }}" method="GET">
                    ...
                    @forelse ($tiposInmueble as $tipo)
                        <x-base.table.tr class="intro-x">
                            <x-base.table.td class="border-b-0 bg-white shadow-[20px_3px_20px_#0000b] first:rounded-l-md last:rounded-r-md dark:bg-darkmode-600">
                                <a class="whitespace-nowrap font-medium" href="{{ route('parametrizacion.tipos_inmueble.show', $tipo) }}">{{ $tipo->nombre }}</a>
                            </x-base.table.td>
                            <x-base.table.td class="border-b-0 bg-white shadow-[20px_3px_20px_#0000b] first:rounded-l-md last:rounded-r-md dark:bg-darkmode-600">
                            ...
                            <x-base.table.td class="relative w-56 border-b-0 bg-white py-0 shadow-[20px_3px_20px_#0000b] before:absolute before:inset-y-0 before:left-0 before:my-auto before:block before:h-8 before:w-px before:bg-slate-200 first:rounded-l-md last:rounded-r-md dark:bg-darkmode-600 before:dark:bg-darkmode-400">
                                <div class="flex items-center justify-center">
                                    <a class="mr-3 flex items-center" href="{{ route('parametrizacion.tipos_inmueble.edit', $tipo) }}">
                                        <x-base.lucide class="mr-1 h-4 w-4" icon="CheckSquare" /> Editar
                                    </a>
                                    <a class="flex items-center text-danger" data-tw-toggle="modal" data-tw-target="#delete-modal-{{ $tipo->id }}" href="javascript:;">
                    ...
                    @empty
                        <x-base.table.tr>
                            <x-base.table.td colspan="4" class="border-b-0 bg-white text-center py-8 text-slate-500 dark:bg-darkmode-600">
                                ...
                                <div class="mt-2">No se encontraron tipos de inmueble.</div>
                            </x-base.table.td>
                        </x-base.table.tr>
                    @endforelse
                </x-base.table.tbody>
        ...
        @if ($tiposInmueble->hasPages())
            <div class="intro-y col-span-12 flex flex-wrap items-center sm:flex-row sm:flex-nowrap">{{ $tiposInmueble->links() }}</div>
        @endif
    </div>
        ...
        <x-base.dialog id="delete-modal-{{ $tipo->id }}">
            <x-base.dialog.panel>
                <div class="p-5 text-center">
                    ...
                    <div class="mt-2 text-slate-500">¿Desea eliminar el tipo <strong>{{ $tipo->nombre }}</strong>?<br />Esta acción se puede revertir.</div>
                </div>
                <div class="px-5 pb-8 text-center">
                    <x-base.button class="mr-1 w-24" data-tw-dismiss="modal" type="button" variant="outline-secondary">Cancelar</x-base.button>
                    <form action="{{ route('parametrizacion.tipos_inmueble.destroy', $tipo) }}" method="POST" class="inline">@csrf @method('DELETE')
