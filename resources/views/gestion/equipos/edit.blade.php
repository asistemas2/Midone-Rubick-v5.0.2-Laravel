        ...
        function cargarTipos(categoriaId) {
            ...
            fetch('{{ route("gestion.equipos.tipos-por-categoria") }}?categoria_equipo_id=' + categoriaId)
                ...
                .then(data => {
                    ...
                    let currentTipoId = {{ old('tipo_equipo_id', $equipo->tipo_equipo_id) ?? 'null' }};
                    select.innerHTML = '<option value="">Seleccionar...</option>';
                    data.forEach(tipo => {
                        let selected = (tipo.id == currentTipoId) ? 'selected' : '';
                        select.innerHTML += `<option value="${tipo.id}" ${selected}>${tipo.nombre}</option>`;
                    });
                });
        }
